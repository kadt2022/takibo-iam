...
- [x] Une cle privee n apparait jamais dans le JWKS, les logs, les metriques ou une erreur. — `SigningKeyRestartAcceptanceTest.given_a_persistent_signing_key_then_no_private_material_leaks_through_any_public_surface` : JWKS fouille sur les huit parametres prives (RFC 7517, RFC 7518) et sur la cle de chiffrement au repos ; `/actuator/env` et `/actuator/metrics` verifies fermes (401) **avant** que leur corps soit fouille ; logs captures par detournement de `System.out`/`System.err` pendant le demarrage et pendant une emission reelle. Un `ListAppender` Logback a d'abord ete essaye : Spring Boot le detache en reconfigurant le logger, la capture restait vide et l'assertion passait sans rien verifier — une garde `isNotBlank()` interdit desormais ce faux positif. Complete `JwkSetEndpointIntegrationTest`, qui couvre la meme surface mais sur la source ephemere du profil de test.
