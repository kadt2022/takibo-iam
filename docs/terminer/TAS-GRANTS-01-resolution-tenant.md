...
- [x] **Regle reformulee a la verification de livraison (2026-09-02).** Formulation d origine : « Un client TMS marque PLATFORM alors que le schema/mapping ne sait pas le representer echoue explicitement; il n est jamais converti silencieusement en SPACE. » Elle supposait qu un client TMS puisse porter un plan, ce qui n est pas le cas : `oauth2_clients` n a aucune colonne `plan`, et `JpaResolvedOAuthClientResolver` construit toujours `ClientPlan.SPACE` (l.100, deja documente dans sa javadoc l.30). La situation decrite est donc **impossible a representer**, et non « refusee ». Regle effective, elle verifiable : **la source TMS ne represente que SPACE ; PLATFORM n existe que dans la source in-memory de developpement.** — `V202601091230__core__init.sql` l.621-622 : `oauth2_clients.org_id` et `space_id` sont `NOT NULL`, or PLATFORM exige les deux a NULL ; aucun client TMS ne peut donc etre PLATFORM. `InMemoryPlatformOAuthClientResolver`, seule source PLATFORM, est limitee aux profils dev/test/ci. La conversion silencieuse en SPACE ne peut pas se produire, faute de plan a convertir.
