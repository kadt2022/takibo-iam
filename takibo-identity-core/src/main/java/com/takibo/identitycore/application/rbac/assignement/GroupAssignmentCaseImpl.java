...
import com.takibo.identitycore.domain.catalogrbac.*;
...
public class GroupAssignmentCaseImpl implements GroupAssignmentCase {
    ...
    private void validateTechnicalGroupScope(TechnicalGroup group,
                                             UUID orgId,
                                             UUID spaceId) {
        ...
        if (scope == TechnicalScope.SYSTEM) {
            if (orgId != null || spaceId != null) {
                throw new IllegalArgumentException("System group " + group.code() + " must not be scoped to org/port");
            }
        }
